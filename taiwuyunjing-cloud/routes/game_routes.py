"""
游戏路由 — 太虚幻境云服务版
适配多用户：API Key 由服务端注入，存档按用户隔离，每轮记录配额
"""
...
from .deps import BASE_DIR, get_engine, get_meta_db, engine_pool, quota_manager
...
def _get_user_save_dir() -> "Path":
    """获取当前用户的存档目录"""
    user_id = get_current_user_id()
    if not user_id or not engine_pool:
        return BASE_DIR / "saves"
    return engine_pool.get_user_save_dir(user_id)


def _record_turn():
    """记录一次游戏轮次到配额"""
    user_id = get_current_user_id()
    if user_id and quota_manager:
        try:
            quota_manager.record_turn(user_id)
        except Exception as e:
            logger.warning("Failed to record turn: %s", e)


# ===== 请求模型（移除了 api_key 等字段，由服务端注入） =====
...
async def list_worlds():
    try:
        db = get_meta_db()
        worlds = db.list_worlds() if db else []
...
async def create_game(req: CreateGameRequest):
    ...
    async with engine._game_lock:
        ...
        if not world_path.exists():
            raise HTTPException(status_code=500, detail="世界模板不存在")
        ...
        world_id = await asyncio.to_thread(
            engine.create_new_game, world_data, player_data, npc_data_list, req.world_name
        )
        state = engine.get_game_state()
        await asyncio.to_thread(engine.save_game, "auto")

    return {"world_id": world_id, "state": state}


@router.post("/load")
async def load_game(req: LoadGameRequest):
    """加载游戏存档（API Key 由服务端注入）"""
    ...
    try:
        ...
        if engine.option_engine and engine.player_state:
            try:
                scene = f"第{engine.world_state.current_day}天 {engine.world_state.current_time}，你在{resolve_location_name(engine.player_state.location, engine.world_state)}"
...
async def player_input(req: PlayerInputRequest):
    ...
    user_id = get_current_user_id()
    if user_id and quota_manager:
        can_play, remaining, limit = quota_manager.check_quota(user_id)
        if not can_play:
            raise HTTPException(status_code=429, detail=f"今日轮数已达上限（{limit}次）")
    ...
    try:
        ...
        try:
            db = get_meta_db()
            if db and result.get("narrative"):
                db.add_narrative(
            ...
            if db and result.get("auto_event"):
                db.add_narrative(
...
async def delete_save(world_id: str):
    ...
    engine = get_engine()
    if engine:
        ok = engine.save_manager.delete_save(world_id)
    else:
        from modules.save_manager import SaveManager
        sm = SaveManager(_get_user_save_dir())
...
async def generate_world(req: GenerateWorldRequest):
    """生成自定义世界（API Key 由服务端注入）"""
    engine = get_engine()
    if not engine:
        raise HTTPException(status_code=503, detail="服务未就绪")

    try:
        # 存储金手指设置到引擎，供后续prompt使用
        ...
        if engine.option_engine and engine.player_state:
            try:
                scene = f"第{engine.world_state.current_day}天 {engine.world_state.current_time}，你在{resolve_location_name(engine.player_state.location, engine.world_state)}，刚刚来到这个世界"
                initial_options = await asyncio.to_thread(engine.option_engine.generate_options, scene, engine.player_state, engine.world_state)
            except Exception as e:
                logger.warning("Failed to generate initial options: %s", e)
                initial_options = engine.option_engine._fallback_options(engine.player_state)
        # [Bug] 将世界简介和初始事件写入 narrative_history
        try:
            from modules.state_history import StateHistoryManager
            save_dir = _get_user_save_dir()
            db_path = save_dir / world_id / "history.db"
...
async def get_narrative_history(world_id: str):
    ...
    if not db_path.exists():
        hist_path = save_dir / world_id / "narrative_history.json"
        jsonl_path = save_dir / world_id / "state" / "narrative_history.jsonl"
