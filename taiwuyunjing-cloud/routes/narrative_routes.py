...
from .deps import BASE_DIR, get_engine, get_meta_db, engine_pool
...
async def list_novel_chapters():
    ...
    if not engine or not engine.current_world_id:
        return {"chapters": []}
    narrative_dir = _get_user_save_dir() / engine.current_world_id / "narrative"
...
async def read_novel_chapter(filename: str):
    ...
    narrative_dir = (_get_user_save_dir() / engine.current_world_id / "narrative").resolve()
    chapter_file = (_get_user_save_dir() / engine.current_world_id / "narrative" / filename).resolve()
    try:
